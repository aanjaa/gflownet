...
prepend_name="4gpu_32cpu_256mem_15650steps_20samples_" #prepend to the name of experiment
...
placement_gpu=0.5 # 0.5 or 1  # num_gpus / num_samples

experiment_names_list=("training_objectives" "buffer" "exploration")
num_trials_list=(12 73 15) # first 12 of the cartesian product, 12 csv files
...
#  echo "#SBATCH --gres=gpu:rtx8000:$num_gpus" >> $script_name
...
#  echo "source ~/gflownet/setup_mila_cluster.sh" >> $script_name
...
#  echo "python raytune.py --experiment_name ${experiment_name} --idx $idx --prepend_name ${prepend_name} --num_cpus $num_cpus --num_gpus $num_gpus --num_samples $num_random_samples --placement_cpu 4 --placement_gpu 0.5" >> $script_name
  echo "python raytune.py --experiment_name ${experiment_name} --idx $idx --prepend_name ${prepend_name} --num_cpus $num_cpus --num_gpus $num_gpus --num_samples $num_random_samples --placement_cpu $placement_cpu --placement_gpu $placement_gpu" >> $script_name
